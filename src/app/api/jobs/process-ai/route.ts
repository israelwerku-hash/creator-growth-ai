...
async function handler(req: Request) {
  ...
  try {
    ...
    const systemPrompt = `You are an elite, high-end creator agency outreach strategist. You MUST respond with valid JSON only. No markdown, no code fences.
Your response must be a single JSON object with exactly these keys:
{
  ...
  "campaignTags": ["tag1", "tag2", "tag3"]
    ...
    const userPrompt = `Generate a highly personalized, compelling, and natural-sounding direct message (DM) for an outreach campaign.
...
2. CRITICAL: You MUST incorporate the fan's specific vault preferences/memories into the outreach message to make it personalized, rather than sending a generic message.
...
Respond with valid json in the exact format required.`;
    ...
    try {
      ...
      try {
        ...
        const parsedJsonTier2 = parseAIJson(completionTier2.choices[0]?.message?.content || "{}");
